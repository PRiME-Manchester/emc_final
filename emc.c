...
void encode_decode_transmit(uint core, uint chip);
void router_setup(uint core, uint chip);
void allocate_memory(uint core, uint chip);
void gen_random_data(uint core, uint chip);
...
/***********************************/
void encode(uint coreID, uint chipID)
{
